<template>
    ...
    <div id="main" class="main" v-if="!loader">
        <div class="header">
...
export default {
  ...
  data() {
      return{
          ...
          loader: true,
          BASE_URL: process.env.VUE_APP_NEO_URL,
...
created(){
    ...
    axios({
    url: this.BASE_URL + 'neo/browse?' + this.API_KEY,
    }).then((res) => {
...
methods: {
    filter_astreoids_by_date(){
        ...
        let req_url = this.BASE_URL + 'feed?start_date=' + formatted_start_date + '&end_date=' + formatted_end_date + '&detailed=false&' + this.API_KEY;
        axios.get(req_url)
                .then((response) => {
